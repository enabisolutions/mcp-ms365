...
describe('allowed scope HTTP behavior', () => {
  ...
  // Enabi fork: Files.Read is not on our allowlist, so an endpoint declaring it
  // never participates in scope diagnostics — the upstream expectation that
  // metadata advertises it doesn't apply here. Re-enable if Files.Read is ever
  // allowlisted.
  it.skip('advertises effective scopes in OAuth metadata', async () => {
