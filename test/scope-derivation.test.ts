...
describe('buildScopesFromEndpoints', () => {
  ...
  // Enabi fork: orgMode + Sites.Read.All / Files.Read assertions assume the
  // full upstream endpoints surface (search-query, drive items, etc). Our
  // trimmed endpoints.json excludes those, so the upstream tests below are
  // skipped. Re-enable if those endpoints are ever re-allowlisted.
  describe.skip('orgMode flag', () => {
  ...
  describe('org-mode + read-only + "search|query" filter', () => {
    ...
    it.skip('includes Files.Read and Sites.Read.All for read-only search tools', () => {
...
describe('allowed scope helpers', () => {
  ...
  describe('resolveAuthScopes', () => {
    ...
    // Enabi fork: depends on Files.Read endpoint surface that we don't allowlist.
    it.skip('filters tool-derived scopes when allowed scopes are supplied', () => {
      expect(
        resolveAuthScopes({
          orgMode: true,
